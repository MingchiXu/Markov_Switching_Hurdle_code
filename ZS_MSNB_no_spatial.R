###########################################
### ZS-MSNB no spatial correlation #########
###########################################
#rm(list=ls())
...
parallel_nimble <- function(dengeeData, dengeeConsts,
  ...
  initsFunction <- function() list(beta0 = rnorm(n=1,mean=0,sd=1),
                                   ...
                                   #r= runif(n=1,min=0,max=100),
                                   X=matrix(rbinom(n=159*384,size=1,prob=.5),nrow=159))
  samples.ZSMSNB.no.spatial <- runMCMC(CdengeeMCMC,  niter =n_iterations,nchains = n_chains,nburnin=warm_up
                              ,samplesAsCodaMCMC = TRUE,thin=n_thin,inits = initsFunction)
  return(samples.ZSMSNB.no.spatial)
}
...
cl <- parallel::makeCluster(3,
                            outfile="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNB_no_spatial.txt")
...
ZSMSNB.samples.no.spatial <- combine_par_mcmc(fit_par)

save(ZSMSNB.samples.no.spatial, file="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNB_no_spatial.RData")

load("/Users/xumingchi/Desktop/hurdle-codes/ZSMSNB_no_spatial.RData")# load the samples
...
samps.ZSMSNB.no.spatial<- data.frame(rbind(ZSMSNB.samples.no.spatial[[1]],
                                ZSMSNB.samples.no.spatial[[2]],
                                ZSMSNB.samples.no.spatial[[3]]))




lppd.z.no.spatial <- 0
pwaic.z.no.spatial <- 0
for(i in 1:159){
  print(i)
  for(t in 2:384){
    # state the parameters
    b0<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("b0.",i,".")]))
    b<-  as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("b.",i,".")]))
    beta1<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("beta1")]))
    beta2<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("beta2")]))
    alpha8<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("alpha8")]))
    alpha9<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("alpha9")]))
    alpha10<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("alpha10")]))
    alpha11<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("alpha11")]))
    gamma3<-  as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("gamma3")]))
    # mup
    mup<- exp(b0)*cases[i,t-1]+exp(b+beta1*(sin.time[t]-mean(sin.time[]))+
                                     beta2*(cos.time[t]-mean(cos.time[])))
    # X
    X.samp<- as.numeric(unlist(samps.ZSMSNB.no.spatial[paste0("X.",i,"..",t,".")]))
    # r 
    ...
    # waic
    lppd.z.no.spatial <- lppd.z.no.spatial + log(mean(dnbinom(x = cases[i,t],
                                        size=r.samp ,prob=p.samp )))
    pwaic.z.no.spatial<- pwaic.z.no.spatial+ var(log(dnbinom(x = cases[i,t],
                                        size=r.samp, prob=p.samp )))
  }
}
