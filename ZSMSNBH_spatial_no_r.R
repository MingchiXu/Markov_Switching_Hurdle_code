#########################################################################
#########  ZS-MSNBH with spatial correlation, without modeling r  #######
#########################################################################
rm(list=ls())
...
parallel_nimble <- function(dengeeData, dengeeConsts,
  ...
  initsFunction <- function() list(beta0 = rnorm(n=1,mean=0,sd=1),
                                   ...
                                   b0=rnorm(n=159,mean=0,sd=.01),
                                   r= runif(n=1,min=0,max=100))
  ZSMSNBH.spatial.no.r <- runMCMC(CdengeeMCMC,  niter =n_iterations,nchains = n_chains,nburnin=warm_up
                             ,samplesAsCodaMCMC = TRUE,thin=n_thin,inits = initsFunction)
  return(ZSMSNBH.spatial.no.r)
}
...
cl <- parallel::makeCluster(3,
                            outfile="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNBH_spatial_no_r.txt")
...
ZSMSNBH.spatial.no.r <- combine_par_mcmc(fit_par)

save(ZSMSNBH.spatial.no.r, file="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNBH_spatial_no_r.RData")


load("/Users/xumingchi/Desktop/hurdle-codes/ZSMSNBH_spatial_no_r.RData")# load the samples
...
samps.ZSMSNBH.no.r<- data.frame(rbind(ZSMSNBH.spatial.no.r[[1]],
                                      ZSMSNBH.spatial.no.r[[2]],
                                      ZSMSNBH.spatial.no.r[[3]]))
...
for(i in 1:159){
  ...
  for(t in 2:384){
    ...
    b0<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("b0.",i,".")]))
    b<-  as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("b.",i,".")]))
    beta1<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("beta1")]))
    beta2<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("beta2")]))
    r.samp<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("r")]))
    
    alpha1<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("alpha1")]))
    alpha2<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("alpha2")]))
    alpha3<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("alpha3")]))
    alpha4<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("alpha4")]))
    alpha5<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("alpha5")]))
    alpha7<- as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("alpha7")]))
    
    
    gamma1<-  as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("gamma1")]))
    gamma2<-  as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("gamma2")]))
    delta1<-  as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("delta1")]))
    delta2<-  as.numeric(unlist(samps.ZSMSNBH.no.r[paste0("delta2")]))
    # mup
    ...
    # waic
    lppd.h.no.r <- lppd.h.no.r + log(mean(dhurdleNegBinMarg(x = cases[i,t],
                                                  p=p01*(1-X[i,t-1])+p11*X[i,t-1],
                                                  lambda= mup, r=r.samp )))
    pwaic.h.no.r<- pwaic.h.no.r+ var(log(dhurdleNegBinMarg(x = cases[i,t],
                                                 p=p01*(1-X[i,t-1])+p11*X[i,t-1],
                                                 lambda= mup, r=r.samp )))
...
waic.zsmsnbh.no.r <- -2*(lppd.h.no.r-pwaic.h.no.r)
