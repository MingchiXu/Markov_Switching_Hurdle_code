########################################################################
######### ZS-MSNB model (with spatial, without modeling r) #############
########################################################################
...
parallel_nimble <- function(dengeeData, dengeeConsts,
  ...
  dengeeCode <- nimbleCode({
    ...
    for(i in 1:159) {
      for(t in 2:384){
        ...
        p[i,t] <- r/(r+(X[i,t])*mup[i,t]) - 1e-10*(1-X[i,t])
        y[i,t] ~ dnegbin(prob=p[i,t],size=r)  
      }
    }
  })
  ...
  dengeeConf$addMonitors(c("beta0","beta1","beta2","beta3","beta4","beta5","beta6",
                           ...
                           "gamma1","gamma2","delta1","delta2","sigma_b0","X",
                           "rho","b","sigma_b","b0","r"))
  ...
  initsFunction <- function()  list(beta0 = rnorm(n=1,mean=0,sd=1),
                                    ...
                                    r= runif(n=1,min=0,max=100),
                                    X=matrix(rbinom(n=159*384,size=1,prob=.5),nrow=159))
  samples.ZSMSNB.spatial.no.r <- runMCMC(CdengeeMCMC,  niter =n_iterations,nchains = n_chains,nburnin=warm_up
                              ,samplesAsCodaMCMC = TRUE,thin=n_thin,inits = initsFunction)
  return( samples.ZSMSNB.spatial.no.r)
}
...
cl <- parallel::makeCluster(3,
                            outfile="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNB_spatial_no_r.txt")
...
samples.ZSMSNB.spatial.no.r <- combine_par_mcmc(fit_par)

save( samples.ZSMSNB.spatial.no.r, file="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNB_spatial_no_r.RData")


load("/Users/xumingchi/Desktop/hurdle-codes/ZSMSNB_spatial_no_r.RData")# load the samples
...
plot(samples.ZSMSNB.spatial.no.r[,c("beta0","rho","beta1")])
...
#---------------------------------
# 2. WAIC for ZS-MSNB model (to be changed)
#---------------------------------


library(RMKdiscrete)

samps.ZSMSNB.no.r<- data.frame(rbind(samples.ZSMSNB.spatial.no.r[[1]],
                                samples.ZSMSNB.spatial.no.r[[2]],
                                samples.ZSMSNB.spatial.no.r[[3]]))




lppd.spatial.no.r <- 0
pwaic.spatial.no.r <- 0
for(i in 1:159){
  print(i)
  for(t in 2:384){
    # state the parameters
    b0<- as.numeric(unlist(samps.ZSMSNB.no.r[paste0("b0.",i,".")]))
    b<-  as.numeric(unlist(samps.ZSMSNB.no.r[paste0("b.",i,".")]))
    beta1<- as.numeric(unlist(samps.ZSMSNB.no.r[paste0("beta1")]))
    beta2<- as.numeric(unlist(samps.ZSMSNB.no.r[paste0("beta2")]))
    r.samp<- as.numeric(unlist(samps.ZSMSNB.no.r[paste0("r")]))
    
    # mup
    mup<- exp(b0)*cases[i,t-1]+exp(b+beta1*(sin.time[t]-mean(sin.time[]))+
                                     beta2*(cos.time[t]-mean(cos.time[])))
    # X
    X.samp<- as.numeric(unlist(samps.ZSMSNB.no.r[paste0("X.",i,"..",t,".")]))
    # p
    p.samp<- r.samp/(r.samp+(X.samp)*mup) - 1e-10*(1-X.samp)
    # waic
    lppd.spatial.no.r <- lppd.spatial.no.r + log(mean(dnbinom(x = cases[i,t],
                                        prob=p.samp, size=r.samp  )))
    pwaic.spatial.no.r<- pwaic.spatial.no.r+ var(log(dnbinom(x = cases[i,t],
                                       prob=p.samp, size=r.samp )))
  }
}

waic.zsmsnb.no.r <- -2*(lppd.spatial.no.r-pwaic.spatial.no.r)
