...
parallel_nimble <- function(dengeeData, dengeeConsts,
  ...
  dengeeConf$addMonitors(c("beta0","beta1","beta2","beta3","beta4","beta5","beta6",
                           ...
                           "gamma3","sigma_b0","rho","b","sigma_b","b0","X"))
...
cl <- parallel::makeCluster(3,
                            outfile="/Users/xumingchi/Desktop/hurdle-codes/ZINB_parallel.txt")
...
save(ZINB.samples, file="/Users/xumingchi/Desktop/hurdle-codes/ZINB.RData")
...
for(i in 1:159){
  ...
  for(t in 2:384){
    ...
    mup<- exp(b0)*cases[i,t-1]+exp(b+beta1*(sin.time[t]-mean(sin.time[]))+
                                     beta2*(cos.time[t]-mean(cos.time[])))
    # X
    X.samp<- as.numeric(unlist(samps.ZINB[paste0("X.",i,"..",t,".")]))
    # r 
    ...
    # waic
    lppd.zinb <- lppd.zinb + log(mean(dnbinom(x = cases[i,t],
                                              size=r.samp ,prob=p.samp )))
    pwaic.zinb<- pwaic.zinb + var(log(dnbinom(x = cases[i,t],
                                              size=r.samp, prob=p.samp )))
  }
}
