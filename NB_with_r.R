...
cl <- parallel::makeCluster(3,
                            outfile="/Users/xumingchi/Desktop/hurdle-codes/NB_parallel.txt")
...
save(NB.samples, file="/Users/xumingchi/Desktop/hurdle-codes/NB.RData")
...
for(i in 1:159){
  ...
  for(t in 2:384){
    ...
    # p
    p.samp<- r.samp/(r.samp+mup) 
    # waic
    lppd.nb <- lppd.nb + log(mean(dnbinom(x = cases[i,t],
                                              size=r.samp ,prob=p.samp )))
    pwaic.nb<- pwaic.nb + var(log(dnbinom(x = cases[i,t],
                                              size=r.samp, prob=p.samp )))
  }
}
