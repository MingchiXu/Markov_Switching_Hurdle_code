############################################
### ZS-MSNBH without spatial correlation ###
############################################
...
parallel_nimble <- function(dengeeData, dengeeConsts,
  ...
  initsFunction <- function() list(beta0 = rnorm(n=1,mean=0,sd=1),
                                   ...
                                   b=rnorm(n=159,mean=0,sd=.01),
                                   b0=rnorm(n=159,mean=0,sd=.01))
  samples.ZSMSNBH.no.spatial <- runMCMC(CdengeeMCMC,  niter =n_iterations,nchains = n_chains,nburnin=warm_up
                             ,samplesAsCodaMCMC = TRUE,thin=n_thin,inits = initsFunction)
  return(samples.ZSMSNBH.no.spatial)
}
...
cl <- parallel::makeCluster(3,
                            outfile="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNBH_parallel_no_spatial.txt")
...
ZSMSNBH.no.spatial.samples <- combine_par_mcmc(fit_par)

save(ZSMSNBH.no.spatial.samples, file="/Users/xumingchi/Desktop/hurdle-codes/ZSMSNBH_no_spatial.RData")


load("/Users/xumingchi/Desktop/hurdle-codes/ZSMSNBH_no_spatial.RData")# load the samples
...
# check convergence
library(coda)
effectiveSize(ZSMSNBH.no.spatial.samples[,c("beta0","beta1","beta2","beta3","beta4","beta5","beta6","sigma_b","sigma_b0","rho",
                                 "alpha1","alpha2","alpha3","alpha4","alpha5","alpha7")])
min(effectiveSize(ZSMSNBH.no.spatial.samples[,c("beta0","beta1","beta2","beta3","beta4","beta5","beta6","sigma_b","sigma_b0","rho")]))

gelman.diag(ZSMSNBH.no.spatial.samples[,c("beta0","beta1","beta2","beta3","beta4","beta5","beta6","sigma_b","sigma_b0","rho",
                               "alpha1","alpha2","alpha3","alpha4","alpha5","alpha7")],multivariate = FALSE)
#randomly check location specific parameters
gelman.diag(ZSMSNBH.no.spatial.samples[,c("b[1]","b[2]","b[3]","b[4]",
                               "b[21]","b[39]","b[112]","b[55]",
                               "b[23]","b[29]","b[15]","b[74]",
                               "b[41]","b[123]","b[16]","b[59]")],multivariate = FALSE)

gelman.diag(ZSMSNBH.no.spatial.samples[,c("b0[1]","b0[2]","b0[3]","b0[4]",
...
summary(ZSMSNBH.no.spatial.samples[,c("beta0","beta1","beta2","beta3","beta4","beta5","beta6","sigma_b","sigma_b0","rho",
                           "alpha1","alpha2","alpha3","alpha4","alpha5","alpha7",
                           "alpha8","alpha9","alpha10","alpha11","gamma1","gamma2",
                           "gamma3")])
...
samps.ZSMSNBH.no.spatial<- data.frame(rbind(ZSMSNBH.no.spatial.samples[[1]],
                                 ZSMSNBH.no.spatial.samples[[2]],
                                 ZSMSNBH.no.spatial.samples[[3]]))
...
lppd.h.no.spatial <- 0
pwaic.h.no.spatial <- 0
for(i in 1:159){
  print(i)
  for(t in 2:384){
    # state the parameters
    b0<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("b0.",i,".")]))
    b<-  as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("b.",i,".")]))
    beta1<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("beta1")]))
    beta2<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("beta2")]))
    
    alpha1<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("alpha1")]))
    alpha2<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("alpha2")]))
    ...
    alpha10<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("alpha10")]))
    alpha11<- as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("alpha11")]))
    
    gamma1<-  as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("gamma1")]))
    gamma2<-  as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("gamma2")]))
    gamma3<-  as.numeric(unlist(samps.ZSMSNBH.no.spatial[paste0("gamma3")]))
    # mup
    ...
    # waic
    lppd.h.no.spatial <- lppd.h.no.spatial + log(mean(dhurdleNegBinMarg(x = cases[i,t],
                                                  p=p01*(1-X[i,t-1])+p11*X[i,t-1],
                                                  lambda= mup, r=r.samp )))
    pwaic.h.no.spatial<- pwaic.h.no.spatial+ var(log(dhurdleNegBinMarg(x = cases[i,t],
                                                 p=p01*(1-X[i,t-1])+p11*X[i,t-1],
                                                 lambda= mup, r=r.samp )))
