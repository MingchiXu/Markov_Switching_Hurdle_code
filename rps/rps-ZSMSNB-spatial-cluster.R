...
#---------------------------------------
#  ZS-MSNB with spatial, modeling r
#---------------------------------------
...
parallel_nimble <- function(dengeeData, dengeeConsts,
  ...
  dengeeCode <- nimbleCode({
    ...
    for(i in 1:N) {
      for(t in 2:T){
        ...
        #y[i,t] ~ dpois(mup[i,t]*X[i,t]+1e-10*(1-X[i,t])) 
      }
    }
  })
  inits <- list(beta0 = rnorm(n=1,mean=0,sd=1),
                ...
                X=matrix(rbinom(n=159*384,size=1,prob=.5),nrow=159))# X size 384/T0
  ...
  dengeeConf$addMonitors(c("beta0","beta1","beta2","beta3","beta4","beta5","beta6",
                           ...
                           "rho","b","sigma_b","b0","X_final"))# "X_final"
...
cl <- parallel::makeCluster(3,
                            outfile=paste0("/lustre03/project/6003552/mingchi/ZSMSNB_parallel_dynamic_",T0,".txt"))
...
gelman.matrix<-gelman.diag(ZSMSNB.samples.dynamic[,c("beta0","beta1","beta2","beta3","beta4","beta5","beta6","sigma_b","sigma_b0","rho",
                                      "alpha1","alpha2","alpha3","alpha4","alpha5","alpha6","alpha7",
                                      "alpha8","alpha9","alpha10","alpha11","gamma1","gamma2",
                                      "gamma3","delta1","delta2")],multivariate = FALSE)


write.csv(gelman.matrix$psrf,paste0("/lustre03/project/6003552/mingchi/rps_result/gelman_ZSMSNB_T0_",as.character(T0),".csv"))
...
rps_zsmsnb<- matrix(0, nrow=159, ncol=K)
...
for (i in 1:159){
  ...
  for (k in 1:K){
    ...
    # calculate the log-score/ rps ##########
    #LS_zip[i,k]<-  -log(mean(dpois(x=cases[i,T0+k], lambda= mup.pred.zip*X.simulated[i,1+k,]+1e-10*(1-X.simulated[i,1+k,])))) #mT=T0
    P_pred <- ecdf(simulated.pred[i,k,])
    rps_zsmsnb[i,k] <- sum((P_pred(seq(0,1000,by=1))-ifelse(cases[i,T0+k]<=seq(0,1000,by=1),1,0))^2)#T=mT
...
#names(dat.LS)[1]<- "LS_ZSMSPH"
row.names(dat.rps)<- c("K=1","K=2","K=3","K=4")

write.csv(dat.rps,paste0("/lustre03/project/6003552/mingchi/rps_result/ZSMSNB_T0_",as.character(T0),".csv"))
