...
cl <- parallel::makeCluster(3,
                            outfile=paste0("/lustre03/project/6003552/mingchi/NBH_parallel_",T0,".txt"))
...
gelman.matrix<-gelman.diag(NBH.samples[,c("beta0","beta1","beta2","beta3","beta4","beta5","beta6",
                                          "alpha4","alpha5","alpha6","alpha7", "sigma_b","sigma_b0","rho",
                                         "alpha8","alpha9","alpha10","alpha11","gamma2",
                                         "gamma3")],multivariate = FALSE)


write.csv(gelman.matrix$psrf,paste0("/lustre03/project/6003552/mingchi/rps_result/gelman_NBH_T0_",as.character(T0),".csv"))
...
# rps
rps_nbh<- matrix(0, nrow=159, ncol=K)
...
for (i in 1:159){
  ...
  for (k in 1:K){
    ...
    rps_nbh[i,k] <- sum((P_pred -ifelse(cases[i,T0+k]<=seq(0,1000,by=1),1,0))^2)#T=mT
    
  }
}

K1<-cbind(mean(rps_nbh[,1]))
K2<-cbind(mean(rps_nbh[,2]))
K3<-cbind(mean(rps_nbh[,3]))
K4<-cbind(mean(rps_nbh[,4]))

dat.rps<-data.frame(rbind(K1,K2,K3,K4))

row.names(dat.rps)<- c("K=1","K=2","K=3","K=4")

write.csv(dat.rps,paste0("/lustre03/project/6003552/mingchi/rps_result/NBH_T0_",as.character(T0),".csv"))
